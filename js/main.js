...
document.addEventListener("DOMContentLoaded", () => {
  const loadHeader = async () => {
    ...
    try {
      const response = await fetch("_header.html"); // Adjust path if _header.html is in a subfolder
      const headerHTML = await response.text();
      headerPlaceholder.innerHTML = headerHTML;
      initializeMenus(); // Call a function to initialize your menus after they are loaded
    } catch (error) {
  ...
  const initializeMenus = () => {
    // --- Top Navbar Responsive Functionality ---
    window.TopBarRespond = function () {
      var topnav = document.getElementById("topNavBar");
      if (topnav.className === "topnav") {
        topnav.className += " responsive";
        document.querySelector(".side-menu").classList.add("show");
        // Animate side menu items
        document
          .querySelectorAll(".side-menu .nav-item")
          .forEach((item, index) => {
            item.classList.add("show");
          });
        document.querySelector(".side-menu .contact").classList.add("show");
      } else {
        ...
        document.querySelector(".side-menu .contact").classList.remove("show");
      }
    };

    // --- Side Menu Dropdown Functionality ---
    ...
    // --- Slider Functionality (from your original index.html) ---
    ...
    sliderNavLinks.forEach((link) => {
      link.addEventListener("click", function (e) {
        ...
        if (targetSlide) {
          // Use scrollIntoView or adjust scrollLeft for horizontal scrolling
          targetSlide.scrollIntoView({
            behavior: "smooth",
            inline: "start", // or 'center', 'end' depending on desired alignment
          });
        }

        // Update active state for slider nav dots
        sliderNavLinks.forEach((navLink) => navLink.classList.remove("active"));
        this.classList.add("active");
      });
    });

    // Set the first slider nav dot as active by default
    if (sliderNavLinks.length > 0) {
      sliderNavLinks[0].classList.add("active");
    }

    // Optional: Auto-scrolling for the slider
    let currentSlide = 0;
  ...
  loadHeader(); // Call loadHeader when the DOM is ready
});

// Holiday Slider functionality (from your original index.html)
const holidayContainer = document.querySelector(".holiday-container");
const nxtBtn = document.querySelector(".nxt-btn");
const preBtn = document.querySelector(".pre-btn");

if (holidayContainer && nxtBtn && preBtn) {
  // Check if elements exist before adding listeners
  let containerWidth = holidayContainer.offsetWidth;
  let cardWidth = document.querySelector(".holiday-card").offsetWidth;

  nxtBtn.addEventListener("click", () => {
    holidayContainer.scrollLeft += cardWidth + 20; // + gap if any
  });
...
// Ensure the logo image for the topnav is set correctly after header load
document.addEventListener("DOMSubtreeModified", () => {
  const topnavLogo = document.querySelector(".topnav .logo-image");
  if (topnavLogo && !topnavLogo.style.backgroundImage) {
    topnavLogo.style.backgroundImage = "url(img/logo.jpg)";
    topnavLogo.style.backgroundSize = "contain";
    topnavLogo.style.backgroundRepeat = "no-repeat";
    topnavLogo.style.backgroundPosition = "center";
  }
});
...
holidaySliders.forEach((slider) => {
  ...
  const setupSlider = () => {
    ...
    const originalCards = Array.from(container.children).filter(
      (el) => !el.classList.contains("clone")
    ); //ensure only originals
    ...
    cardWidth =
      firstCardForWidth.getBoundingClientRect().width +
      marginLeft +
      marginRight;

    if (cardWidth <= 0) {
      console.error(
        "Card width is zero or negative. Cannot setup slider:",
        slider
      );
    ...
    console.log(
      `Initial Setup: numOriginals=${numOriginals}, cardWidth=${cardWidth.toFixed(
        2
      )}, clonesPrepended=${clonesPrepended}, initialScroll=${initialScroll.toFixed(
        2
      )}`
    );
    container.scrollTo({
    ...
    console.log(
      `Total width: ${totalWidth.toFixed(
        2
      )}, Client width: ${container.clientWidth.toFixed(
        2
  ...
  const handleScrollEnd = () => {
    ...
    console.log(
      `Boundaries: firstOriginalStart=${firstOriginalStart.toFixed(
        2
      )}, firstAppendedStart=${firstAppendedStart.toFixed(2)}`
    );
    ...
    if (container.scrollLeft >= firstAppendedStart - tolerance) {
      ...
      console.log(
        `Wrap Forward Detected: Jumping from ${container.scrollLeft.toFixed(
          2
        )} to ${newScrollLeft.toFixed(2)}`
      );
    ...
    //check if we landed in the prepended clones area
      ...
      const newScrollLeft = container.scrollLeft + numOriginals * cardWidth;
      console.log(
        `Wrap Backward Detected: Jumping from ${container.scrollLeft.toFixed(
          2
        )} to ${newScrollLeft.toFixed(2)}`
      );
      container.style.scrollBehavior = "auto"; //disable smooth for the jump
  ...
  if ("onscrollend" in window) {
    ...
    console.warn(
      "Browser does not support 'scrollend'. Infinite loop jump might be noticeable or slightly delayed (fallback needed)."
    );
  ...
  galleryItems.forEach((item) => {
    item.addEventListener("click", (event) => {
      ...
      if (fullsizeUrl) {
        ...
        console.error(
          "Data attribute data-fullsize-src not found on image:",
          item
        );
      }
    });
        ...
        if (response.ok) {
          ...
          response
            ...
            .catch((error) => {
              ...
              formStatus.textContent =
                "Oops! There was a problem submitting your form. Server error.";
              formStatus.style.color = "red";
